...
def home(request):
    ...
    for plan in plans:
        ...
        if key:
            meal_map[day][key] = [item.menu.name for item in plan.diet_menus.all()]
    ...
    for pred in predictions:
        ...
        if pred.meal_type not in prediction_map[day]:
            prediction_map[day][pred.meal_type] = pred.predicted_count
    ...
    selected_predictions = (
        AttendancePrediction.objects.filter(prediction_date=selected_date)
        ...
        selected_prediction_rows.append(
            {
                ...
                "meal_label": pred.get_meal_type_display(),
